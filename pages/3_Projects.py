...
with col2:
    st.markdown("""<h3 class="title_text">ML Maintenance Prédictive</h3>""", unsafe_allow_html=True)
    st.image("images/ML_maintenance_predictive.jpg")
    st.markdown("""<p class="text_on_background">Développement d'un modèle de machine learning pour prédire les pannes d'équipements industriels.</p>""", unsafe_allow_html=True)
    if st.button("🔍 Voir le projet", key="project_ml_maintenance"):
        st.switch_page("pages/Project_ML_Maintenance_Predictive.py")

with col4:
    st.markdown("""<h3 class="title_text">NLP BM25</h3>""", unsafe_allow_html=True)
    st.image("images/NLP_BM25.jpg")
    st.markdown("""<p class="text_on_background">Système de recherche et d'analyse de texte utilisant l'algorithme BM25.</p>""", unsafe_allow_html=True)
...
with col2:
    st.markdown("""<h3 class="title_text">Computer Vision Welding</h3>""", unsafe_allow_html=True)
    try:
    ...
    st.markdown("""<p class="text_on_background">Détection automatique de défauts de soudure par vision par ordinateur.</p>""", unsafe_allow_html=True)
    if st.button("🔍 Voir le projet", key="project_cv_welding"):
        st.switch_page("pages/Projet_CV_welding.py")

with col4:
    st.markdown("""<h3 class="title_text">Analyse de Données RTE</h3>""", unsafe_allow_html=True)
    try:
        st.image("images/outil_simu.jpg")
    except:
        st.markdown("""<div style="height: 200px; background: var(--color_encart); border-radius: 10px; display: flex; align-items: center; justify-content: center; color: white; font-size: 3rem;">⚡</div>""", unsafe_allow_html=True)
    st.markdown("""<p class="text_on_background">Analyse et visualisation de données du réseau électrique français.</p>""", unsafe_allow_html=True)
...
with col2:
    st.markdown("""<h3 class="title_text">ML_comparative_study</h3>""", unsafe_allow_html=True)
    try:
    ...
    st.markdown("""<p class="text_on_background" style="text-align: center;">Développement de ce portfolio interactif avec Streamlit et déploiement cloud.</p>""", unsafe_allow_html=True)
